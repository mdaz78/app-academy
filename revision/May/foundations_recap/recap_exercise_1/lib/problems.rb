...
def all_vowel_pairs(words)
  ...
  while (i < words.length)
    ...
    while (j < words.length)
      pairing_word = "#{words[i]} #{words[j]}"

      if all_vowels_found?(pairing_word)
        pairs << pairing_word
      end
...
def all_vowels_found?(word)
  vowels = "aeiou"

  vowels.split("").all? do |vowel|
    word.include? vowel
  end
end
  ...
  def substrings(length = nil)
    ...
    (0...self.length).each do |start_index|
      (start_index...self.length).each do |end_index|
        word = self[start_index..end_index]
        subs << word
      end
    end
