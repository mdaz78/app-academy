...
# of 3 is 6 because 1 + 2 + 3 = 6, the summation of 6 is 21
# because 1 + 2 + 3 + 4 + 5 + 6 = 21. Write a method
# summation_sequence that takes in a two numbers: start and length.
...
# the summation of the previous element. You can assume that length is not zero.

def summation_sequence(start, length)
  sequence = [ start ]
  (1...length).each do |idx|
    previous_element = sequence[idx - 1]
    next_element = get_summation(previous_element)
